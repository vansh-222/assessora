 ...
 * Body: { action: 'update-name', name: string }
 *    or { action: 'change-password', currentPassword: string, newPassword: string }
 */
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (body.action === 'change-password') {
      ...
      await User.findByIdAndUpdate(session.user.id, { password: hash });
...
/**
 * GET /api/settings — Export all user data as JSON
 */
export async function GET() {
  ...
  try {
    ...
    const [user, assessments, attempts] = await Promise.all([
      User.findById(userId).select('name email createdAt').lean(),
      Assessment.find({ userId }).select('-analysis.rawText').lean(),
...
/**
 * DELETE /api/settings — Delete user account and all associated data
 */
